...
copyDirs = ["/scripts", "/openloader", "/config", "/mods", "/defaultconfigs"]
serverCopyDirs = ["/scripts", "/config", "/mods", "/defaultconfigs"]
modURLlist = []
...
with open(basePath + "/buildOut/modlist.html", "w") as file:
    data = "<html><body><h1>ADG Omega 5 Modlist</h1><ul>"
    for mod in modlist:
        data += "<li>" + mod + "</li>"
    data += "</ul></body></html>"
    file.write(data)
...
vanilla = basePath + "/buildOut/server/minecraft_server.1.16.5.jar"

if not os.path.isfile(vanilla):
    with open(basePath + "/buildOut/server/minecraft_server.1.16.5.jar", "w+b") as jar:
